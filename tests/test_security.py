...
class TestPIIDetector:
    ...
    def test_detects_email(self):
        found = self.pii_detector.detect("Contact me at john@example.com")
        assert "email" in found

    def test_detects_phone(self):
        found = self.pii_detector.detect("Call me at 555-123-4567")
        assert "phone" in found

    def test_detects_ssn(self):
        found = self.pii_detector.detect("SSN: 123-45-6789")
        assert "ssn" in found

    def test_detects_credit_card(self):
        found = self.pii_detector.detect("Card: 4111-1111-1111-1111")
        assert "credit_card" in found

    def test_no_pii_returns_empty(self):
        found = self.pii_detector.detect("Hello, how are you?")
        assert len(found) == 0
